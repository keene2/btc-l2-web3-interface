...
import { lastHashAtom, mintBalanceAtom } from '@/features/unisatCore';
...
export const MenusView: React.FC = ({
  ...
  const [lastTx, setLastTx] = useAtom(lastHashAtom);
  const [visible, setVisible] = React.useState(false);
  const [current, setCurrent] = useState('mail');

  const res = useQuery({
    enabled: Boolean(address),
    queryKey: ['get-balance-info', address],
    queryFn: async () => {
      try {
        const res = await http.post(`/balance`, { address });
        setMintBalance(res.data.balances[0]);
        setLastTx(res.data?.lastHash);
        return res.data;
      } catch (e) {
